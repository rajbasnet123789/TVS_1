...
const navItems = [
  ...
  { label: 'Chickens', path: '/chickens', icon: <ChickenIcon />, permission: 'chickens:read' },
...
export function Sidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  // Poll cameras and chickens to compute health index
  useEffect(() => {
    const fetchHealth = () => {
      Promise.all([
        api.get('/cameras').catch(() => ({ data: [] })),
        api.get('/chickens').catch(() => ({ data: [] })),
      ]).then(async ([camerasRes]) => {
  ...
  const content = (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', bgcolor: '#ffffff' }}>
      <Toolbar sx={{ px: 3, py: 2.5, display: 'flex', alignItems: 'center', gap: 1.5 }}>
        <Box component="img" src="/tvs_logo.png" alt="TVS Logo" sx={{ height: 22, objectFit: 'contain' }} />
        <Typography 
          variant="caption" 
          sx={{ 
      ...
      {/* COOP STATUS card at the bottom */}
      <Box sx={{ p: 2.5, mt: 'auto', borderTop: '1px solid #f1f5f9' }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
            <Box sx={{ position: 'relative', display: 'inline-flex' }}>
              <svg width="80" height="80" viewBox="0 0 80 80">
                <circle cx="40" cy="40" r={radius} fill="transparent" stroke="#f1f5f9" strokeWidth={strokeWidth} />
                <circle 
                  cx="40" 
                  cy="40" 
                  r={radius} 
              ...
              >
                ...
                <Typography variant="caption" sx={{ fontSize: '0.475rem', fontWeight: 800, color: '#64748b', letterSpacing: '0.04em', mt: 0.1 }}>
                  HEALTH INDEX
                </Typography>
              </Box>
            </Box>
